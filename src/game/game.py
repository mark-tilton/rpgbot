...
class Game:
    ...
    def start_adventure(
            self,
            user_id: int,
            zone_id: int) -> Optional[AdventureReport]:
        with self.storage_model as t:
            start_time = floor(time.time())
            current_adventure = self.storage_model.get_current_adventure(
                user_id)
            report = None
        ...
        with self.storage_model as t:
            ...
            for adventure_step in report.adventure_steps:
                for item_id, quantity in (
                        adventure_step.items_gained.items.items()):
